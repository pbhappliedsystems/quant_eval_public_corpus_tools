...
BUILDER_VERSION = "build_datasets.py 2.4.0"
...
def build_d7(bundles, outdir):
    ...
    for b in bundles:
        ...
        rows.append({
            ...
            "hardware_backend_scope": rc.get("hardware_backend_scope", ""),
        })
...
def dataset_card(did, name, stats, runs, concept, version, fixture_info,
    ...
    A("- **Runs on the Modal substrate record `seed` status `unsupported`** — "
      "the deployed method signature accepts no seed — so those runs are not "
      "exactly reproducible. Local runs applied a fixed seed.")
